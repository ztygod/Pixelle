...
import {usePreviewStore} from "@/features/preview-runtime/model/preview-store";
import {getPreviewFrameLabel} from "@/features/preview-runtime/services/preview-session-service";
    ...
    <aside className="workspace-panel flex min-h-[560px] min-w-0 flex-col overflow-hidden rounded-lg xl:h-full xl:min-h-0">
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-b border-white/10 px-3 py-2">
        ...
        <div className="flex shrink-0 items-center gap-1">
          <Button aria-label="Desktop preview" onClick={() => setViewport("desktop")} size="icon-sm" variant="ghost">
            <Monitor size={15} />
          </Button>
          <Button aria-label="Tablet preview" onClick={() => setViewport("tablet")} size="icon-sm" variant="ghost">
            <Tablet size={15} />
          </Button>
          <Button aria-label="Mobile preview" onClick={() => setViewport("mobile")} size="icon-sm" variant="ghost">
            <Smartphone size={15} />
          </Button>
